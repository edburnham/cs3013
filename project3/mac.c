...
void* nodeProcess(void* nodeInfo){
	...
	while(totalTime != EXP_DURATION){
		...
		//for testing orints out channel messages
		/* for(i = 0; i < 1000; i++){
			  if(channel_1[i].nodeID != 0){
				  printf("ch1 mess: %s\n", channel_1[i].transMess);
			  }
		  }
		  for(i = 0; i < 1000; i++){
			  if(channel_6[i].nodeID != 0){
				  printf("ch6 mess: %s\n", channel_6[i].transMess);
			  }
		  }
    	  for(i = 0; i < 1000; i++){
			  if(channel_11[i].nodeID != 0){
				  printf("ch11 mess: %s\n", channel_11[i].transMess);
			  }
		  }*/
		/*reading from each channel*/ 
		//printf("numNodesInRange: numNodesInRange %d\n", numNodesInRange);
		for(i = 0; i < numNodesInRange;i++){
			//if(nodesInRange[i].nodeID != 0){printf("node in range: %d\n", nodesInRange[i].nodeID);}
		}
		...
		for(i = 0; i < numNodesInRange; i++){
			if(nodesInRange[i].nodeID != 0){            
				...
				for(j = 0; j < localChannelCount1; j++){
					//puts("here1");
					sem_wait(&channel_1lock);
					//if(channel_1[i].nodeID != 0){printf("checking if equal:   %d     %d\n", nodesInRange[i].nodeID, channel_1[j].nodeID);}
