...
int PIDSwap(int PID) {//not evicting PID passed in, evicting next PID in RR, evicting all pages and page table at the same time
	...
	if((int)memory[hardwareReg[RR_PIDeviction]*16] != PID){//if the PID is not our own
		hardDisk = fopen("hardDisk.bin", "wb");//evict full page table
		fwrite(&memory[hardwareReg[RR_PIDeviction]*16], sizeof(char), 1, hardDisk);//PID
		for(i = 1; i < 16; i++){
			//fprintf(hardDisk, "%d\n", (int)memory[hardwareReg[RR_PIDeviction]*16 + i]);//VPN
			fwrite(&memory[hardwareReg[RR_PIDeviction]*16 + i], sizeof(char), 1, hardDisk);
			diskLen++;
		}		
		...
		for(i = 0; i < 4; i++){	
			if(tempF == -1){
				//fprintf(hardDisk, "%d\n", 0);
				fwrite(&zero, sizeof(char), 1, hardDisk);
				diskLen++;
			}else{
				//fprintf(hardDisk, "%d\n", (int)memory[tempF*16 + i]);
				fwrite(&memory[hardwareReg[tempF*16 + i]], sizeof(char), 1, hardDisk);
				diskLen++;
			}
		}
		...
		fclose(hardDisk);
		printf("Swapped frame %lu to disk at offset %lu.\n", hardwareReg[RR_PIDeviction], diskLen - 16);
		freeList[hardwareReg[RR_PIDeviction]] = 0;
		freeList[tempF] = 0;	
		hardwareReg[RR_PIDeviction] = diskLen + 4;
		return 1;
	}else{
		memory[hardwareReg[RR_PIDeviction]*16]
	}
...
int swapIn(int PID, int VPN, int openPFNTable, int openPFNPage){
	...
	PFNpage = memory[openPFNTable*16 + (VPN*4 + 2)];
	
	for(i = 0; i < 16; i++){
		if(i == (VPN*4 + 2)){
			memory[openPFNTable*16 + i] = (char)openPFNPage;
		}else{
			memory[openPFNTable*16 + i] = diskLines[i];
		}	
	}
	
	memset(diskLines, 0 ,sizeof(diskLines));
	fseek(hardDisk, PFNpage*4, SEEK_SET);
	fread(diskLines, 16, 1, hardDisk);
	memcpy(&memory[openPFNPage*16], &diskLines, sizeof(diskLines));
	freeList[openPFNPage] = 1;
	freeList[openPFNTable] = 1;
